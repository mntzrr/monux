...
- **A hosted hotspot (no cable)**: `sudo monux system setup --hotspot` on the server creates a 'monux-direct' WiFi hotspot (checked against the card's AP support). Clients then provision themselves: the server hands the credentials to each approved client over the encrypted, mutually-authenticated KVM connection (they never travel any other way), and the client creates and joins the profile on its own — nothing to copy. The client re-associates to the hotspot — its old WiFi profile returns automatically when the hotspot is off — and its internet keeps working, NATed through the server. Discovery prefers the direct link automatically. Opt out on the client with `--no-auto-hotspot`, or join manually with the printed `sudo monux system setup --hotspot-join '<ssid>' '<psk>'` (e.g. where polkit blocks user-level profile creation). Remove the profile with `sudo monux system setup --hotspot off` on either machine (or let `monux system uninstall` do it). Notes: the hotspot shares the router's channel on a single-radio card (it removes the router's buffering and other clients from the path, not the band's airtime contention); and **a VPN tunnel active on the server (Mullvad/WireGuard/OpenVPN) usually breaks the NAT that gives hotspot clients internet** — the KVM link works, but the client's internet doesn't, so disconnect the VPN while using the hotspot (setup warns when it detects one).
