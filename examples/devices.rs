...
impl devicewatch::DeviceHandler for StubHandler {
    fn handle_device_stream(&mut self, mut stream: evdev::EventStream) -> Result<task::JoinHandle<()>> {
        task::Builder::new().name(format!("device: {:?}", stream.device().name())).spawn(async move {
            ...
            let device_name = stream.device().name().unwrap_or("(Unnamed device)").to_string();
...
fn main() -> Result<()> {
    ...
    let mut keyboard = deviceoutput::keyboard().context("Failed to init virtual device, are you root?")?;
    let mut mouse = deviceoutput::mouse()?;
    let mut touchpad = deviceoutput::touchpad()?;

    // Sleep for a bit, otherwise events can be missed. Devices need a bit of time to come up.
    thread::sleep(Duration::from_secs(1));

    for _ in 0..50 {
        keyboard.emit(&[InputEvent::new(EventType::KEY, Key::KEY_R.code(), 1)]).unwrap();
        keyboard.emit(&[InputEvent::new(EventType::KEY, Key::KEY_R.code(), 0)]).unwrap();
    }
